...
null}if(d.lp){m=d.lp.multiply(d.lp.toUsedLength(d.w,d.h),d.options.sizeAdjust);for(e=0;e<u.length;e++)if(k=Math.ceil(m)+"px",r="-"+Math.ceil(m)+"px",0!==u[e].elements.length){l=u[e].elements[u[e].start_elem];var p=u[e].elements[u[e].end_elem];if(l===p&&(elementBoundingRect=l.node.getBoundingClientRect(),0==elementBoundingRect.width||0==elementBoundingRect.height))continue;"lr"===d.ipd?(l.node.style.marginLeft=r,l.node.style.paddingLeft=k):"rl"===d.ipd?(l.node.style.paddingRight=k,l.node.style.marginRight=
r):"tb"===d.ipd&&(l.node.style.paddingTop=k,l.node.style.marginTop=r);"lr"===d.ipd?(p.node.style.paddingRight=k,p.node.style.marginRight=r):"rl"===d.ipd?(p.node.style.paddingLeft=k,p.node.style.marginLeft=r):"tb"===d.ipd&&(p.node.style.paddingBottom=k,p.node.style.marginBottom=r)}"tb"===d.bpd?(d.removePaddingElement.style.paddingLeft=0,d.removePaddingElement.style.paddingRight=0):(d.removePaddingElement.style.paddingTop=0,d.removePaddingElement.style.paddingBottom=0);d.lp=null}for(m=0;m<u.length;m++)for(e=
u[m],k=1;k<e.elements.length;){r=e.elements[k-1].node;l=e.elements[k].node;if("SPAN"===r.tagName&&"SPAN"===l.tagName&&r._isd_element===l._isd_element){r.textContent+=l.textContent;for(p=0;p<l.style.length;p++){var t=l.style[p];if(0<=t.indexOf("border")||0<=t.indexOf("padding"))r.style[t]=l.style[t]}l.parentElement.removeChild(l);r=!0}else r=!1;r?e.elements.splice(k,1):k++}m=[];for(r=0;r<u.length;r++)for(l=0;l<u[r].elements.length;l++)if(e=u[r].elements[l].node,k=a(e,m,!1))e.style.backgroundColor=
k;for(e=0;e<m.length;e++)m[e].style.backgroundColor="";if(d.flg){e=D(g.getBoundingClientRect(),d);m=e.before;e=e.after;k=Math.sign(e-m);for(r=0;r<=u.length;r++){l=0===r?Math.round(m):r===u.length?Math.round(e):Math.round((u[r-1].after+u[r].before)/2);if(0<r&&u[r-1])for(t=0;t<u[r-1].elements.length;t++){var H=u[r-1].elements[t];p=k*(l-H.after)+"px";"lr"===d.bpd?H.node.style.paddingRight=p:"rl"===d.bpd?H.node.style.paddingLeft=p:"tb"===d.bpd&&(H.node.style.paddingBottom=p)}if(r<u.length)for(t=0;t<u[r].elements.length;t++)H=
u[r].elements[t],p=k*(H.before-l)+"px","lr"===d.bpd?H.node.style.paddingLeft=p:"rl"===d.bpd?H.node.style.paddingRight=p:"tb"===d.bpd&&(H.node.style.paddingTop=p)}d.flg=null}}"region"===b.kind&&"tb"===d.bpd&&d.enableRollUp&&0<b.contents.length&&"after"===b.styleAttrs[n.byName.displayAlign.qname]&&(h(d,g,u,null),b=new c(""===b.id?"_":b.id,u),d.currentISDState[b.id]=b,d.previousISDState&&b.id in d.previousISDState&&0<d.previousISDState[b.id].plist.length&&1<b.plist.length&&b.plist[b.plist.length-2].text===
